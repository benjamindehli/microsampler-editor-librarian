...
import { onPatternsProgress } from "components/patterns/patterns.js";
import { followSelect } from "components/sample-editor/waveform.js";
import { onOpEvent } from "components/utility/utility.js";
import { state } from "functions/state.js";
import { tick } from "functions/ticker.js";
import { $, fmtSigned } from "functions/util.js";
...
export function subscribeEvents() {
    ...
    es.onmessage = (e) => {
        ...
        if (evt.type === "progress") return evt.op === "patterns" && onPatternsProgress(evt.done, evt.total);
        ...
        const who = evt.sample != null ? `S${evt.sample + 1}` : isFx ? "FX" : evt.obj === 0 ? "BANK" : `obj${evt.obj}`;
        if (evt.obj === 0 && state.bank) {
            // bank panel edits (obj 0)
            if (evt.param === 16) {
                // BPM × 10
        ...
        const shown = isFx
            ? fmtSigned(dec14(evt.value))
            : BIPOLAR.has(evt.param)
              ? fmtSigned(dec14(evt.value))
              : evt.param === PARAM.TUNE
                ? tuneDisplay(evt.value)
                : evt.value;
        tick(`← ${who} #${evt.param} = ${shown}`);
        if (evt.sample != null)
            // panel edits update the cache
            cacheParam(
                evt.sample,
                evt.param, // for EVERY slot, selected or not
