...
export function renderMeter() {
    ...
    const ptrn = (state.bank.seq_lengths || []).reduce((a, b) => a + (b === 0xff ? 0 : b) * 0x200, 0);
...
export const fmtMem = (b) => (b >= 1 << 20 ? (b / (1 << 20)).toFixed(1) + "MB" : `${Math.round(b / 1024)}KB`);
